...
class dmpR3:
    ...
    def amplify_window(self, t_start, t_stop, amplify_factor):
        for i in range(3):
            self.dmp_array[i].amplify_window(t_start, t_stop, amplify_factor)


    # Function to save the DMP weights (w)
    def saveDmpParams(self, filename):
        weights = [dmp.W.tolist() for dmp in self.dmp_array]  
        #dmp.W: access the weight matrix 'W' of each DMP object and then convert each numpy array to a list

        with open(filename, 'w') as file:
           json.dump(weights, file)  #Save the list of the weights to a JSON file
    ...
    def plotPerformance(self, dt, p_array):
        ...
        for i in range(3):
            ...
            axs.plot(t, pDMP[i, :], color='red', linewidth=1.0)
            ...
            if i==2:
                ...
                lgnd = axs.legend(['$DMP$','$demo$'],fontsize=11,ncol=2,loc="upper right")
...
#----- gets an error from psi somewhere when running this function -----
#----- connection between dmpR3 and dmp here with the get_state_dot function -----
#----- changed the if statement regarding the forcing term value in get_state_dot (in dmp.py) -----
    ...
    def plotResponse(self, dt, p0, iterations):
            ...
            fig = plt.figure(figsize=(10, 8))
            #fig.suptitle('DMP Response', fontsize=12)  # Adding a title to the figure
