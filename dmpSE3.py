...
class dmpSE3:
    def __init__(self, N_in, T_in, kernelType_in = 'Gaussian', xType_in = 'linear', a_x_in = 1.0, a_in=20.0, b_in=0.8, tau_in=1.0):
        ...
        # Create DMPs
        self.dmp_translation = dmpR3(N_in, T_in, kernelType_in = 'Gaussian', xType_in = 'linear', a_x_in = 1.0, a_in=20.0, b_in=0.8, tau_in=1.0)  # A dmpR3 object for 3D position trajectories
        self.dmp_orientation = dmpSO3(N_in, T_in, kernelType_in = 'Gaussian', xType_in = 'linear', a_x_in = 1.0, a_in=20.0, b_in=0.8, tau_in=1.0)  # A dmpSO3 object for quaternion-based rotation trajectories
    
    
    # Function for window amplification of a part of a motion
    def amplify_window(self, t_start, t_stop, amplify_factor):
        self.dmp_translation.amplify_window(t_start, t_stop, amplify_factor)  # Amplify the window of the translation DMP
        self.dmp_orientation.amplify_window(t_start, t_stop, amplify_factor)  # Amplify the window of the orientation DMP
    ...
    def train(self, dt, p_array, Q_array, plotPerformance = False):
        ...
        if plotPerformance: 
            print('test')
            self.plotPerformance(dt, p_array, Q_array)
