...
class dmp:
    ...
    def amplify_window(self, t_start, t_stop, amplify_factor):
        n_start = math.floor(t_start * self.N /self.T)
        n_stop = math.floor(t_stop * self.N /self.T)

        self.W[n_start:n_stop+1] = amplify_factor * self.W[n_start:n_stop+1]
    ...
    def get_state_dot(self, x, y, z, customScale = False, scalingTerm = 1):
        ...
        # get the values of kernel base for this x
        ...
        if self.kernelType == 'sinc':
            ...
            # Added a check to ensure that sum_Psi is not zero. When it's zero, a small epsilon value is added to avoid division by zero.
            if sum_Psi == 0:
               #print("Warning: Sum of Psi is zero, adding epsilon to avoid division by zero.")
               sum_Psi += 1e-10  
            f = self.W @ Psi / sum_Psi
