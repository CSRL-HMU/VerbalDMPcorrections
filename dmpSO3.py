...
#SO(3) - Special Orthogonal Group in 3D (3d rotation group)
...
class dmpSO3:
    ...
    # Function for window amplification of a part of a motion
    def amplify_window(self, tstart, tstop, ampfactor):
        for i in range(3):
            self.dmp_array[i].amplify_window(tstart, tstop, ampfactor)
...
# The train method processes the input quaternion trajectory. 
# The quaternion trajectory Q_array is used to compute the desired rotational velocities. 
# These velocities are used as targets for training the intenal DMP basis function weights.
...
# e_log refers to the logarithmic map for rotations. 
# It is a mathematical operation that maps a rotation from the SO(3) to its corresponding Lie algebra (which is represented as a vector in R^3)

# The logarithmic error (e_log) is computed for each time step relative to the target quaternion. The rotational errors are used to train the DMP weights.

# will probab;y retrive the end effectors position using get_actual_tcp_pose() and then use the orientation to train the DMP weights.
